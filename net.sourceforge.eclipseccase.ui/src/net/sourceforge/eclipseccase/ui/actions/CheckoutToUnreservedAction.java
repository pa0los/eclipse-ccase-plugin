 ...
 * 
 */
public class CheckoutToUnreservedAction extends ClearCaseWorkspaceAction {
	...
	public void execute(IAction action) {
		...
		IWorkspaceRunnable runnable = new IWorkspaceRunnable() {
			...
			public void run(IProgressMonitor monitor) throws CoreException {
				try {
					...
					beginTask(monitor, "Changing checkout state to unreserved...", resources.length);
					...
					if (provider != null) {
						...
						for (int i = 0; i < resources.length; i++) {
							...
							provider.unreserved(new IResource[] { resource }, 0, subMonitor(monitor));
		...
		executeInBackground(runnable, "Changing checkout state to unreserved");
	}

	private static final String DEBUG_ID = "CheckoutToUnreservedAction";
